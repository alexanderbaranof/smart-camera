...
def yolo_eval(yolo_outputs, anchors, num_classes, image_shape, max_boxes=100,
              score_threshold=.01, iou_threshold=.01):
    """Evaluate YOLO model on given input and return filtered boxes."""
    num_layers = len(yolo_outputs)
    anchor_mask = [[6, 7, 8], [3, 4, 5], [0, 1, 2]] \
        if num_layers == 3 else [[3, 4, 5], [0, 1, 2]]  # default setting
...
def compute_tp_fp_fn(boxes_true, boxes_pred, iou_thresh=0.1):
