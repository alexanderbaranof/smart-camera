...
def handle_predictions(predictions, confidence=0.1, iou_threshold=0.1):
...
print(fps)
class_names = load_coco_names("coco_classes.txt")
draw_boxes(boxes, classes, scores, img, class_names, (height, width), True)
img.save("output.jpg")

df = pd.read_csv('datetime_series.csv')
df.append({'date-time':str(t_start),
            'count':boxes.shape[0]})